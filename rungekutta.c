...
void rungekutta(void){
    ...
    for(int kx=0; kx<x_split; kx++){
        for(int ky=0; ky<y_split; ky++){
            int k = kx + ky * x_split;
            kari[k + 0 * split] = k1[k] = Q1[k];
            kari[k + 1 * split] = k2[k] = Q2[k];
            kari[k + 2 * split] = k3[k] = Q3[k];
            kari[k + 3 * split] = k4[k] = Q4[k];
        }
    }
    ...
    for(int kx=0; kx<x_split; kx++){
        for(int ky=0; ky<y_split; ky++){
            int k = kx + ky * x_split;
            Q1[k] = k1[k] = kari[k + split * 0];
            Q2[k] = k2[k] = kari[k + split * 1];
            Q3[k] = k3[k] = kari[k + split * 2];
            Q4[k] = k4[k] = kari[k + split * 3];
        }
    } 
